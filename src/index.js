...
function firstMovieDetails(){
  ...
  const endpointA = "http://localhost:3000/films";
  ...
  function buyTicket(movie){
    ...
    if(movie.capacity === movie.tickets_sold){
      purchaseTicket.textContent = "Sold Out";
    }
  };
    ...
    .then((data) => {
      function pickTitle(title){
        ...
        li.querySelector("#destroy").addEventListener("click", (e) => {
          li.remove();
          deleteMovieInList(title.id);
        })
      };

      data.forEach(title => pickTitle(title));
